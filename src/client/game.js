...
function onServerMessage(msg){
  switch(msg.type){
    ...
    case "peer_left":
      ...
      break;
    case "net_pong":
      // Block B: ответ сервера на наш net_ping — экстрагируем RTT и
      // server-clock offset. Пара id-stamped, переживает out-of-order.
      Game._netPong(msg);
      break;
  }
}
...
// Block E (Apr 2026): онлайн снова включён. «Играть» → startMatchmaking,
// 5с лобби-таймер автоматически фолбэкается в бот-матч если пары не нашлось
// (см. onLobbyTimeoutFallback). Если сервер успел прислать matched_pending,
// клиент переключается на 20с «preparing match» под Hathora cold-start.
// Сетевая телеметрия (Block B) пишется в data/online-diag.jsonl на сервере.
...
$("btn-replay").addEventListener("click", ()=>{
  // Block E (Apr 2026): «Играть снова» возвращает в matchmaking, как
  // первый клик «Играть». Бот-матч повторяется только если предыдущий
  // тоже был ботовый (state.mode === "bot") — тогда быстрый рестарт без
  // запроса очереди. Это сохраняет UX «попрактиковаться против бота
  // быстро», но для PvP даёт полный re-pair с возможным новым партнёром.
  if(state.mode === "bot"){
    state.session = { matchId: "b-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2,8), startedAt: Date.now(), seq: 0 };
    state.stakes = null;
    requestStakes(state.session.matchId);
...
const Game = (function(){
  ...
  let snapAcc = 0;
  // Block B (Apr 2026): host-auth snapshot seq + matchStart. Гость использует
  // seq для lostSnapPct (детект TCP HoL), srvTickMs (=now-matchStartMs) — для
  // ordering и измерения end-to-end задержки. Сбрасываются на старте матча.
  let snapSeq = 0;
  let matchStartMs = Date.now();
  ...
  // Block B (Apr 2026) NetDiag: lost-snap по seq-gap + RTT через ws-ping.
  // _lostSnapCount растёт когда applySnapshot видит s.seq > prev+1 (TCP HoL
  // retransmit либо sender-side backpressure-drop). RTT — EMA по id-stamped
  // ping/pong парам, переживает out-of-order. Clock.offsetMs := (server_now -
  // performance_now) — Clock.now() возвращает приблизительный server wall-clock.
  let _lastSnapSeq = 0;
  let _lostSnapCount = 0;
  let _seenSnapCount = 0;
  let _rttEmaMs = 0;
  let _maxRttMs = 0;
  let _diagSent = false;
  ...
  let _pingNextId = 1;
  let _pingTimer = 0;
  function netSendPing(){
    if(!state.ws || state.ws.readyState !== 1) return;
    const id = _pingNextId++;
    _pingInFlight.set(id, performance.now());
    try { state.ws.send(JSON.stringify({ type: "net_ping", id, t: Date.now() })); } catch(_){}
    if(_pingInFlight.size > 16){
      const firstKey = _pingInFlight.keys().next().value;
      _pingInFlight.delete(firstKey);
    }
  }
  function netHandlePong(msg){
    const id = (msg && msg.id) | 0;
    const sentPerfNow = _pingInFlight.get(id);
    if(sentPerfNow == null) return;
    _pingInFlight.delete(id);
    const recvPerfNow = performance.now();
    const rtt = Math.max(0, Math.round(recvPerfNow - sentPerfNow));
    _rttEmaMs = (_rttEmaMs === 0) ? rtt : Math.round(_rttEmaMs * 0.7 + rtt * 0.3);
    if(rtt > _maxRttMs) _maxRttMs = rtt;
    if(typeof msg.srv === "number"){
      const serverNow = msg.srv + Math.round(rtt / 2);
      const offset = serverNow - recvPerfNow;
      Clock.offsetMs = (Clock.offsetMs === 0)
        ? offset
        : Math.round(Clock.offsetMs * 0.7 + offset * 0.3);
    }
  }
  function netStartPings(){
    if(_pingTimer) return;
    netSendPing();
    _pingTimer = setInterval(netSendPing, 2500);
    if(_pingTimer && typeof _pingTimer.unref === "function") _pingTimer.unref();
  }
  function netStopPings(){
    if(_pingTimer){ clearInterval(_pingTimer); _pingTimer = 0; }
    _pingInFlight.clear();
  }
  function netSendDiag(){
    if(_diagSent) return;
    if(!state.ws || state.ws.readyState !== 1) return;
    // Бот-матч и пустые сессии не интересны для онлайн-диагностики.
    if(state.mode === "bot") return;
    if(_snapTotalCount === 0 && state.mode !== "host") return;
    _diagSent = true;
    const j = _snapDiagStats();
    const total = _lostSnapCount + _seenSnapCount;
    const lostPct = total > 0 ? (_lostSnapCount / total) : 0;
    try {
      state.ws.send(JSON.stringify({
        type: "net_diag",
        matchId: state.activeMatchId || null,
        mode: state.mode,
        netMode: state.netMode || null,
        matchOver: !!state.matchOver,
        sessionMs: matchStartMs ? (Date.now() - matchStartMs) : 0,
        snapTotal: _snapTotalCount,
        lostSnaps: _lostSnapCount,
        lostPctX100: Math.round(lostPct * 10000),
        bigSnaps: _bigSnapCount,
        extraps: _extrapCount,
        rttEmaMs: _rttEmaMs,
        maxRttMs: _maxRttMs,
        jitterP50Ms: j ? Math.round(j.p50 * 1000) : null,
        jitterP95Ms: j ? Math.round(j.p95 * 1000) : null,
        jitterP99Ms: j ? Math.round(j.p99 * 1000) : null,
        jitterMaxMs: j ? Math.round(j.maxEver * 1000) : null,
        frameTimeAvg: Math.round(frameTimeAvg * 100) / 100,
        frameTimeP95: Math.round(_frameTimeP95() * 100) / 100,
  ...
  function resetMatch(){
    ...
    _rttEmaMs = 0; _maxRttMs = 0; _diagSent = false;
    renderDelay = 0.12;
  ...
  function start(){
    ...
    // Block B: запускаем ping-цикл при старте матча, останавливаем в stop().
    // Бот-матч пингует тоже — Railway-ws всё равно открыт, минимальный трафик
    // (1 frame/2.5s ≈ 40 Б/с), но даёт live-RTT в overlay для отладки.
    netStartPings();
    last = Clock.now();
    acc = 0;
  ...
  function stop(){
    // Block B: пакуем net_diag перед сбросом state — _diagSent guard
    // защищает от повторной отправки если stop() вызван несколько раз.
    try { netSendDiag(); } catch(_){}
    netStopPings();
    state.inGame = false;
    state.matchOver = false;
    state.session = null;
  ...
  function broadcastSnapshot(){
    ...
    try {
      const seq       = ++snapSeq;
      const srvTickMs = (Date.now() - matchStartMs) >>> 0;
  ...
  function applySnapshot(s){
    ...
    _snapCount++;
    _snapTotalCount++;
    // Block B: lostSnap по seq-gap. seq монотонно растёт у авторитета (host
    // в legacy-host-auth, room-server в server-auth). Дырка >1 = либо TCP
    // retransmit съел, либо sender backpressure-drop'нул. На TCP до клиента
    // потерь почти не бывает (только при разрыве/HoL >RTO), но ради
    // диагностики sender-drop'ов считаем.
    if(typeof s.seq === "number"){
      if(_lastSnapSeq > 0 && s.seq > _lastSnapSeq + 1){
        _lostSnapCount += (s.seq - _lastSnapSeq - 1);
      }
  ...
  function _debug(){
    ...
    return {
      ...
      // Block B NetDiag: lostSnap по seq-gap + RTT/Clock-offset из ws-ping.
      // lostPctX100 = lost/(lost+seen) × 10000 (две дроби после запятой).
      // rttEmaMs / maxRttMs: end-to-end ping/pong через Railway state.ws.
      lostSnaps: _lostSnapCount,
      lostPctX100: (_lostSnapCount + _seenSnapCount > 0)
        ? Math.round(10000 * _lostSnapCount / (_lostSnapCount + _seenSnapCount))
        : 0,
      rttEmaMs: _rttEmaMs,
      maxRttMs: _maxRttMs,
  ...
  return { start, stop, refreshOverlay, triggerEmote, applySnapshot, endByForfeit, _debug, _netPong: netHandlePong };
})();
