...
// Block C (Apr 2026): дефолт 15с → 6с. На свежем регионе cold-start обычно
// 2-4с; 6с покрывает p95. Если не успели — лучше быстрее уйти в pairLocal
// (gracefully), чем держать пользователя в "preparing match" 15 секунд.
// Pre-warm пул в server.js делает большинство pair-запросов 0-cost (room
// уже готов), так что POLL_TIMEOUT срабатывает только на miss.
const POLL_TIMEOUT_MS  = Number(process.env.HATHORA_POLL_TIMEOUT_MS) || 6000;
