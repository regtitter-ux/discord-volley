...
// Корень репозитория. server.js живёт в src/server/, поэтому два уровня
// вверх. Все file-ops и spawn'ы используют ROOT, не __dirname.
...
// Каталоги, которые express отдаёт как статику в корне URL. Клиентский код
// + shared-модули (physics/codec) — оба должны быть достижимы как /game.js
// и /physics.js соответственно. Один и тот же список используется для
// автогенерации cache-busting regex ниже: добавил новый JS-файл в src/client
// или src/shared — он автоматически получит ?v=BUILD_ID без правки server.js.
const STATIC_DIRS = [
...
const INDEX_HTML = (function(){
  let html = fs.readFileSync(path.join(ROOT, "src/client/index.html"), "utf8");
  const versioned = [];
  for (const d of STATIC_DIRS){
    for (const f of fs.readdirSync(d)){
      if (/\.(js|css)$/i.test(f) && f !== "index.html") versioned.push(f);
    }
  }
  const escaped = versioned.map(f => f.replace(/\./g, "\\.")).join("|");
  const versionRegex = new RegExp(`(href|src)="(${escaped})"`, "g");
  html = html.replace(versionRegex, (_m, attr, file) => `${attr}="${file}?v=${BUILD_ID}"`);
...
// Static-mount: src/client (UI), src/shared (physics/codec — браузеру нужны
// под теми же URL'ами, что и серверу, чтобы wire-format и физ-константы были
// единой правдой), public/assets под /assets/. Множественные mount'ы express
// проходит в порядке регистрации, первый match выигрывает — конфликтов имён
// между client/shared нет.
const staticOpts = {
  index: false,
  setHeaders: (res, p) => {
    if (/\.(webp|png|jpg|jpeg|svg|woff2|ico|gif)$/i.test(p)) {
...
for (const d of STATIC_DIRS){
  app.use(express.static(d, staticOpts));
}
...
const { ShadowRegistry }  = require("../room/shadowsim");
const hathoraClient       = require("./hathora-client");
const hathoraRegions      = require("./hathora-regions");
const { signRoomToken, verifyWebhook } = require("../room/room-auth");
...
// Block C (Apr 2026): pre-warm pool. Активируется при первом enqueue —
// пока pair ещё формируется, в фоне крутится createRoom в FALLBACK_REGION.
// Когда pair готов и регион совпал — pairHathora получает уже-готовый
// {host, port} без 2-6с polling. После consume сразу заводим следующую
// тёплую. Idle TTL ниже Hathora room idle-timeout (5 мин), чтобы не упасть
// в expired-комнату. Off-by-default через DV_HATHORA_PREWARM=0.
const HATHORA_PREWARM = process.env.DV_HATHORA_PREWARM !== "0" && DV_ROOMS === "hathora" && !DV_LOCAL_ROOMS;
const HATHORA_PREWARM_TTL_MS = Number(process.env.HATHORA_PREWARM_TTL_MS) || 4 * 60 * 1000;
const _hathoraWarmPool = new Map();  // region → { promise, resolved, createdAt }

function _ensureWarm(region){
  if (!HATHORA_PREWARM || !ROOM_SECRET) return;
  if (_hathoraWarmPool.has(region)) return;
  const entry = { promise: null, resolved: null, createdAt: Date.now() };
  _hathoraWarmPool.set(region, entry);
  entry.promise = (async () => {
    try {
      const room = await hathoraClient.createRoom({ region });
      entry.resolved = room;
      console.log(`[ws] hathora-prewarm ready region=${region} room=${room.roomId} at=${room.host}:${room.port}`);
    } catch(e){
      console.warn(`[ws] hathora-prewarm failed region=${region}: ${e && e.message || e}`);
      _hathoraWarmPool.delete(region);
    }
  })();
}

async function _consumeWarm(region){
  if (!HATHORA_PREWARM) return null;
  const entry = _hathoraWarmPool.get(region);
  if (!entry) return null;
  // Не ждём, если ещё warming — пусть pairHathora пойдёт обычным путём.
  // Альтернатива (await entry.promise) добавила бы tail-latency: warm в
  // полёте ≠ warm готов; пара получит ту же 6с задержку.
  if (!entry.resolved) return null;
  // Age-out: тёплая дольше PREWARM_TTL_MS — Hathora idle-таймер (5 мин)
  // близко, безопаснее не использовать. Дропаем + кикаем замену.
  if (Date.now() - entry.createdAt > HATHORA_PREWARM_TTL_MS){
    _hathoraWarmPool.delete(region);
    _ensureWarm(region);
    return null;
  }
  _hathoraWarmPool.delete(region);
  setImmediate(() => _ensureWarm(region));
...
async function spawnLocalRoomServer({ roomId, matchId }){
  ...
  const args = [
    path.join(ROOT, "src/room/room-server.js"),
    "--port",   String(port),
    "--secret", ROOM_SECRET
  ];
...
async function pairHathora(host, guest){
  ...
  try {
    if (DV_LOCAL_ROOMS){
      ...
      // Block C: пробуем тёплую комнату нужного региона. Hit → 0-cost
      // (без polling), miss → обычный createRoom (≤6с с новым timeout'ом).
      room = await _consumeWarm(region);
      if (!room){
        room = await hathoraClient.createRoom({ region });
      } else {
        console.log(`[ws] hathora-prewarm hit region=${region} room=${room.roomId}`);
      }
    }
  } catch (e){
...
async function onQueue(ws){
  ...
  // Block C: триггерим pre-warm в момент queue. Если для этого игрока
  // сразу найдётся партнёр — warm может ещё не дойти, и pairHathora
  // упадёт в обычный createRoom (≤6с). Если он один в очереди и партнёр
  // придёт через 5-10с — warm к этому моменту готов, pair мгновенен.
  // Регион выбираем pairHathora'ой по обоим country, тут warm-im только
  // для самого вероятного сценария (геомедианный fallback).
  _ensureWarm(HATHORA_FALLBACK_REGION);
  if (ws._country){
    const r = hathoraRegions.regionForCountry(ws._country);
    if (r && r !== HATHORA_FALLBACK_REGION) _ensureWarm(r);
  }
...
wss.on("connection", async (ws, req) => {
  ...
  ws.on("message", async (raw, isBinary) => {
    ...
    switch (msg.type) {
      ...
      case "net_ping": {
        // Block B: эхо немедленно. Клиент стампит `id` для пары send→pong;
        // `t` возвращаем как было (RTT клиент сам считает по своему таймеру),
        // `srv` — наш Date.now() для NTP-подобной синхронизации Clock.offsetMs.
        const id = (msg && Number.isInteger(msg.id)) ? msg.id : 0;
        const t  = (msg && typeof msg.t === "number") ? msg.t : 0;
        send(ws, { type: "net_pong", id, t, srv: Date.now() });
        break;
      }
      case "net_diag": {
        // Block B: пост-матч диагностика от клиента → один JSONL line в
        // data/online-diag.jsonl. Сериализуем с серверным ts + userId, чтобы
        // grep'аемо было фильтровать по сессии. Размер файла не контролируем
        // тут — append-only, ротацию пусть делает logrotate/админ.
        try {
          const line = JSON.stringify({
            ...
            mode:       msg.mode       || null,
            netMode:    msg.netMode    || null,
            matchOver:  !!msg.matchOver,
            sessionMs:  msg.sessionMs  | 0,
            snapTotal:  msg.snapTotal  | 0,
            lostSnaps:  msg.lostSnaps  | 0,
            lostPctX100:msg.lostPctX100| 0,
            bigSnaps:   msg.bigSnaps   | 0,
            extraps:    msg.extraps    | 0,
            rttEmaMs:   msg.rttEmaMs   | 0,
            maxRttMs:   msg.maxRttMs   | 0,
            jitterP50Ms:msg.jitterP50Ms ?? null,
            jitterP95Ms:msg.jitterP95Ms ?? null,
            jitterP99Ms:msg.jitterP99Ms ?? null,
            jitterMaxMs:msg.jitterMaxMs ?? null,
            frameTimeAvg: typeof msg.frameTimeAvg === "number" ? msg.frameTimeAvg : null,
            frameTimeP95: typeof msg.frameTimeP95 === "number" ? msg.frameTimeP95 : null,
            lowQuality: !!msg.lowQuality
          }) + "\n";
          fs.appendFile(path.join(DATA_DIR, "online-diag.jsonl"), line, () => {});
        } catch(_){}
        break;
      }
    }
